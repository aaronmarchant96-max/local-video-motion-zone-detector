...
This is the same basic event extraction pattern used in my other analysis tools:

```text
video input
frame comparison
zone or region filtering
...
Generate a synthetic security camera style demo video
Load named motion zones from zones.json
Compare grayscale frame differences
Calculate a motion score per zone
Apply a configurable threshold
Apply cooldown timing to reduce duplicate alerts
Save annotated event snapshots
Write structured event logs to JSONL
...
Blur frames slightly to reduce noise
Compare each processed frame to the previous processed frame
Crop comparison regions using configured zones
Sum pixel differences inside each zone
Trigger an event when the zone score crosses the threshold
Save an annotated snapshot with the active zone highlighted
Log event metadata to JSONL
Example Zones
...
Add contact sheets for event review
Add per zone thresholds
Add minimum object area filtering
Add background subtraction
Add CSV summaries
Add a demo GIF
Add optional real webcam or RTSP input later
Add a manual review labeling tool
Disclaimer
